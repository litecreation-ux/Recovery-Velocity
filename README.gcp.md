...
Grant the Cloud Run runtime service account `Secret Manager Secret Accessor` on
those secrets. The Cloud Build service account needs permission to build/push
to Artifact Registry and deploy Cloud Run (and permission to act as the runtime
service account when one is configured).
...
request timeout for SSE, minimum one instance, and concurrency 40.

In Clerk, add `_APP_ORIGIN` to allowed origins and configure the corresponding
sign-in/sign-up redirect URLs. If using the Cloud Run URL, include its exact
HTTPS origin (no path). Optional integrations may additionally require
