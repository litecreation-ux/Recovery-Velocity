...
export default function Communications() {
  ...
  return <div className="p-5 md:p-8 space-y-6 max-w-7xl mx-auto w-full">
    ...
    <div className="grid lg:grid-cols-2 gap-6">
      ...
      <form onSubmit={sendRadio} className="border border-border bg-card p-5 space-y-4"><h2 className="font-semibold uppercase text-sm flex gap-2 items-center"><Radio className="w-4 h-4 text-primary" />Radio traffic log</h2><div className="grid grid-cols-2 gap-3"><Input value={station} onChange={(e) => setStation(e.target.value)} placeholder="Station / call sign" required /><select value={direction} onChange={(e) => setDirection(e.target.value)} className="h-10 bg-background border border-input px-3 text-sm"><option value="outgoing">Outgoing</option><option value="incoming">Incoming</option></select></div><Textarea value={radioBody} onChange={(e) => setRadioBody(e.target.value)} placeholder="Traffic body" required /><Button type="submit" variant="outline" className="w-full">Record radio traffic</Button></form>
    </div>
